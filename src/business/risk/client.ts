...
export interface RiskDirectClientOptions {
  ...
  onBusinessCapabilities?: (capabilities: RiskBusinessCapabilities) => void;
}

export interface RiskBusinessCapabilities {
  sharedTextMemoization: 'optimized' | 'unoptimized';
  functions: Record<string, { optimized: boolean; maxEntries: number | null }>;
}

export interface RiskDirectRuntimeStatus {
  ready: boolean;
  processId?: number;
...
export class RiskDirectClient implements RiskService {
  ...
  private ready = false;
  private capabilities?: RiskBusinessCapabilities;
  ...
  /** Metadata from this client's current child, never a fresh diagnostic process. */
  runtimeStatus(): RiskDirectRuntimeStatus {
    if (this.closing || !this.ready || !isChildRunning(this.child)) return { ready: false };
    return {
      ready: true,
      ...(Number.isInteger(this.child.pid) && this.child.pid! > 0 ? { processId: this.child.pid } : {}),
      ...(this.capabilities ? { businessCapabilities: structuredClone(this.capabilities) } : {}),
    };
  }
  ...
  private assertOpen(): void {
    if (this.closing) throw new RiskServiceError('risk-service 已关闭', 'direct-process');
  }
  ...
  private async ensureStarted(): Promise<void> {
    ...
    this.lines.on('line', (line) => {
      if (generation === this.childGeneration && child === this.child) this.handleLine(line);
    });
  ...
  private handleLine(line: string): void {
    ...
    if (message.type === 'ready') {
      ...
      const capabilities = businessCapabilities(message.business_capabilities);
      this.capabilities = capabilities;
      if (capabilities) {
        try { this.options.onBusinessCapabilities?.(structuredClone(capabilities)); }
        catch { /* Diagnostics cannot affect readiness. */ }
      }
      this.ready = true;
      this.startResolve?.();
...
function businessCapabilities(value: unknown): RiskBusinessCapabilities | undefined {
  if (!isRecord(value)) return undefined;
  const status = value.shared_text_memoization;
  if (status !== 'optimized' && status !== 'unoptimized') return undefined;
  if (!isRecord(value.functions)) return undefined;
  const functions: RiskBusinessCapabilities['functions'] = {};
  for (const name of ['clean_text', 'normalize_product_name']) {
    const item = value.functions[name];
    if (!isRecord(item) || typeof item.optimized !== 'boolean') return undefined;
    const maxEntries = item.max_entries;
    if (maxEntries !== null && (typeof maxEntries !== 'number' || !Number.isSafeInteger(maxEntries) || maxEntries <= 0)) {
      return undefined;
    }
    if (item.optimized && maxEntries === null) return undefined;
    functions[name] = { optimized: item.optimized, maxEntries };
  }
  const allOptimized = Object.values(functions).every((item) => item.optimized);
  if ((status === 'optimized') !== allOptimized) return undefined;
  return { sharedTextMemoization: status, functions };
}
