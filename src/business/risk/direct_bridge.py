...
def shared_text_capability(checker: Any) -> dict[str, Any]:
    """Report sanitized shared-source optimization metadata only."""
    functions: dict[str, dict[str, Any]] = {}
    for name in ("clean_text", "normalize_product_name"):
        row = {"optimized": False, "max_entries": None}
        try:
            function = getattr(checker, name, None)
            marked = getattr(function, "__risk_text_memoized__", False) is True
            cache_info = getattr(function, "cache_info", None)
            if callable(function) and marked and callable(cache_info):
                bound = cache_info().maxsize
                if type(bound) is int and bound > 0:
                    row = {"optimized": True, "max_entries": bound}
        except Exception:
            # Optional diagnostics must never prevent readiness or a business call.
            # Unknown/broken metadata is reported conservatively, without its text.
            pass
        functions[name] = row
    optimized = all(item["optimized"] is True for item in functions.values())
    return {
        "shared_text_memoization": "optimized" if optimized else "unoptimized",
        "functions": functions,
    }
...
class DirectRiskService:
    def __init__(self, service_dir: Path, state_dir: Path) -> None:
        ...
        self.web = web
        self.business_capabilities = shared_text_capability(self.checker)
...
def main() -> int:
    ...
    write_message({
        "type": "ready",
        "startup_timings": service.startup_timings,
        "business_capabilities": service.business_capabilities,
    })
