...
type ThreadSessionMap = Record<string, ThreadSessionRecord>;

export interface ThreadSessionStoreOptions {
  maxAgeMs?: number;
  maxEntries?: number;
  now?: () => Date;
}
...
export class ThreadSessionStore {
  ...
  constructor(
    ...
    options: ThreadSessionStoreOptions = {},
  ) {
...
function validateSessions(value: unknown, file: string): ThreadSessionMap {
  ...
  const sessions: ThreadSessionMap = {};
