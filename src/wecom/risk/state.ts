...
type StoredConversationState =
  | { status: 'active'; state: RiskConversationState }
  | { status: 'expired' };
...
/** Single owner for all WeCom risk continuation state and card callbacks. */
export class RiskStateRegistry {
  ...
  has(conversationKey: string): boolean {
    return this.states.get(conversationKey)?.status === 'active';
  }
  ...
  private setConversation(conversationKey: string, state: RiskConversationState): void {
    this.delete(conversationKey);
    const stored: StoredConversationState = { status: 'active', state };
    this.states.set(conversationKey, stored);
    const timer = setTimeout(() => {
      if (this.states.get(conversationKey) !== stored) return;
      this.stateTimers.delete(conversationKey);
      // Drop the potentially large draft/query payload but retain a tiny marker
      // so the next numeric/freeform reply can receive an explicit expiry notice.
      this.states.set(conversationKey, { status: 'expired' });
      this.pruneExpired();
    }, this.ttlMs);
    timer.unref?.();
