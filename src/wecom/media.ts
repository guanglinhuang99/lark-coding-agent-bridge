...
function outputInstruction(): string {
  return [
    ...
    '用户明确要求回传已上传的原附件并写出文件名时，可以链接该附件的原始绝对路径；否则不要回传输入附件。桥接程序会核验来源和内容。',
    '文件链接只是发送请求，实际发送由桥接程序完成；不要声称已发送或已回传，请写“准备回传”。',
    '若创建了需要回传给用户的文件，请在最终回答中使用绝对路径 Markdown 链接；生成文件仅链接工作区内确实需要回传的文件。',
  ].join('\n');
}
