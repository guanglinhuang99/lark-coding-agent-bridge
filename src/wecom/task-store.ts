...
export interface WeComTaskRecord {
  id: string;
  operationKey: string;
  conversationKey: string;
  kind: WeComTaskKind;
  status: WeComTaskStatus;
  label: string;
  attempts: number;
  createdAt: string;
  updatedAt: string;
...
interface WeComTaskDiskState {
  schemaVersion: 1;
  tasks: Record<string, WeComTaskRecord>;
  operations: Record<string, string>;
}

export interface WeComTaskStoreOptions {
  maxAgeMs?: number;
  maxEntries?: number;
  now?: () => Date;
}

export interface WeComTaskClaim {
  accepted: boolean;
  replayed: boolean;
  task: WeComTaskRecord;
}

export interface WeComTaskStoreSnapshot {
  total: number;
  queued: number;
  running: number;
  done: number;
...
export class WeComTaskStore {
  private tasks: Record<string, WeComTaskRecord> = {};
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Invalid WeCom task store ${this.file}: JSON is damaged; the original file was preserved`,
        { cause: err },
      );
    }
  ...
  async claimInbound(messageId: string, conversationKey: string): Promise<WeComTaskClaim> {
    const operationKey = hashOperationKey(messageId);
  ...
  async annotate(
    taskId: string,
    patch: Partial<Pick<WeComTaskRecord, 'kind' | 'label'>>,
  ): Promise<void> {
  ...
  private persist(): Promise<void> {
    const state: WeComTaskDiskState = {
      schemaVersion: 1,
      tasks: this.tasks,
...
function validateState(value: unknown, file: string): WeComTaskDiskState {
  ...
  const tasks: Record<string, WeComTaskRecord> = {};
...
function damaged(file: string, key?: string): Error {
  return new Error(
    `Invalid WeCom task store ${file}${key ? `: damaged entry ${JSON.stringify(key)}` : ''}; the original file was preserved`,
  );
}

function cloneTask(task: WeComTaskRecord): WeComTaskRecord {
  return { ...task };
}

function isTaskStatus(value: unknown): value is WeComTaskStatus {
  return value === 'queued' || value === 'running' || value === 'done' || value === 'failed' || value === 'interrupted';
}

function isTaskKind(value: unknown): value is WeComTaskKind {
  return value === 'message' || value === 'command' || value === 'codex' || value === 'risk' || value === 'attachment';
}
