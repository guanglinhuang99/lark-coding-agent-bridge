...
describe('core architecture boundary', () => {
  ...
  it('routes both WeCom agent entry paths through the shared executor', async () => {
    ...
    expect(cli).not.toContain('codex.run(');
    expect(cli.match(/startWeComAgentRun\(runExecutor/g)).toHaveLength(1);
    expect(cli).toContain('createRiskBusinessRuntime(');
    const runtime = await readFile('src/runtime/risk-business.ts', 'utf8');
    expect(runtime).toContain('await getExecutor().submit(');
    expect(runtime).toContain('permit: input.pool.currentPermit()');
  ...
  it('binds every risk intent AI path to the conversation stop lifecycle', async () => {
    ...
    const runtime = await readFile('src/runtime/risk-business.ts', 'utf8');
    expect(runtime).toContain('AbortSignal.any([request.signal, shutdown.signal])');
    expect(runtime).toContain('signal: combined, permit: input.pool.currentPermit()');
    expect(cli).toContain('riskApplication.cancelScope(key)');
    expect(runtime).toContain('await collectRiskIntent(run,');
    const application = await readFile('src/business/risk/application.ts', 'utf8');
    expect(application).toContain('this.options.analyze({ key, originalText, previous, correction, signal })');
    expect(application).toContain('request.abort()');
    const collector = await readFile('src/business/risk/analyzer.ts', 'utf8');
    expect(collector).toContain("addEventListener('abort', stop");
    expect(collector).toContain('await run.stop()');
    expect(runtime).toContain('await run?.stop()');
