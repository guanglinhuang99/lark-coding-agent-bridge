...
共享代码不意味着共享权限或会话。会话键包含渠道、机器人/实例、聊天范围、用户和工作区。企业微信在渠道应答和排队前捕获共享 RiskIngress。飞书在任何异步聊天/话题识别前记录公共状态版本，再在落账前绑定 RiskIngress；晚于消息到达时生成或替换的草稿不能被该确认使用。版本标记是单次使用的服务端对象，不读取客户端提供的版本，也不因其他会话更新而误拒绝原稿。旧卡片和更换后的草稿不能互相替代。重复投递、落账失败及正常完成路径均释放捕获请求。

修改交易即撤销旧确认；修改失败不能继续执行旧稿。状态读取直接核对截止时间，不依赖定时器及时触发。关闭/停止禁止继续生成业务状态或正常展示迟到成功结果；已提交给后端的只读操作不保证可以物理中断。

确认消费或交互取消后，共享状态层保留只含结束原因与截止时间的轻量提示标记，不保留可执行草稿或业务结果。标记按完整业务会话键隔离，默认最多 2000 项、保留 5 分钟（与状态 TTL 相同），重复确认不延长保留时间。在该窗口内，后续“确认/confirm/好的/1”等确认词返回“已处理，不会重复测算”或“已取消”，不会进入普通 Agent；失败结果也不被说成测算成功。

结束标记不占待确认状态、不阻止新测算。明确开始新的普通聊天后让回普通 Agent，但迟到的旧普通消息不能删除更新的结束标记。标记过期、被容量淘汰或进程重启后不再保留原业务归属，不是永久的跨重启去重记录。两个渠道的 `/stop` 均展示共享取消回执；普通 Agent 的原有停止处理仍保留。停止不等待取消回执：飞书立即交回普通停止处理，退出时最多等候取消回执 1000ms；企业微信先停止活跃任务，再有限等待取消回执 1000ms。超时只记录通知未完成，不重试业务、不假称回执已送达。平台已经发出的网络请求不保证能物理取消。

`/new`、企业微信 `/reset` 和新会话按钮使用共享 reset/resetScope，不复用 stop 的终态保留语义；它们清除本范围的活动草稿及结束标记，其他工作区不受影响。结束标记也使用公共单调版本号：早于标记到达、晚于标记才完成聊天/话题识别的普通消息不能清除新标记。
...
企业微信在公共工厂中指定 `legacyPrefix: WECOM`，继续接受对应 `WECOM_RISK_*`。非空公共值优先；空字符串或仅空白字符视为未设置，仍回退到有效旧配置。非空但非法的公共数值会报错，不会被旧值掩盖。建议逐步迁移为统一的 `RISK_*`，不要同时配置互相矛盾的值。
