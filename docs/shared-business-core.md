...
结束标记不占待确认状态、不阻止新测算。明确开始新的普通聊天后让回普通 Agent，但迟到的旧普通消息不能删除更新的结束标记。标记过期、被容量淘汰或进程重启后不再保留原业务归属，不是永久的跨重启去重记录。两个渠道的 `/stop` 均展示共享取消回执；普通 Agent 的原有停止处理仍保留。停止不等待取消回执：飞书立即交回普通停止处理并在退出时等待已发回执结束，企业微信先停止活跃任务再发送回执。
