...
Historical operational note at PR acceptance time: the pre-existing `ai.lark-channel-bridge.bot.codex` LaunchAgent also contains an extra script argument after the installed `lark-channel-bridge` entrypoint, causing an `unknown command` restart loop. It was not edited as part of this surgical PR acceptance. The isolated authenticated validation invoked the PR entrypoint with the required proxy-cleared environment instead of changing that plist.

Subsequent maintenance on 2026-09-07: the workspace was fast-forwarded to `2a5cfec78cfd605dbe9c908dd0ca63535c0bd972`, built, and the local CLI's `restart --profile codex` rewrote and reloaded only the Lark codex LaunchAgent. The loaded and on-disk arguments matched the canonical five-argument definition; bot connection was confirmed, PID was 36940, `runs` stayed at 1 and stderr did not grow during the approximately 82-second observation. This resolves the historical LaunchAgent issue above, but does not change the PR's tested revision or constitute a production-state migration/rollback drill. The subsequent dual-platform startup work is recorded separately in `dual-bot-validation.md`.
