...
| `WECOM_CODEX_MODEL_OPTIONS` | — | optional comma-separated model allowlist for `/model`; the current and startup models remain available automatically |
...
| `WECOM_CODEX_POST_DONE_EXIT_GRACE_MS` | `5000` | abnormal-cleanup fallback grace after a terminal Codex event: normal successful runs call `finish()` immediately, while this window is only used to detect and stop a child process that still does not exit after normal cleanup |
